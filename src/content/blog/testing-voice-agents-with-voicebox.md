...
- **Artifacts.** Pass a `record_dir` and the session writes a stereo WAV (tester on one channel, bot on the other, a convention borrowed from EVA), plus `events.json` and `metrics.json` with per-turn response latency, dead-air gaps and talk-over windows. A test that does not leave evidence behind is just a demo.
