...
- **Artifacts.** Pass a `record_dir` and the session writes the artifacts you need to review it afterwards: `events.json` (the whole event log), `metrics.json` (per-turn response latency, dead-air gaps, talk-over windows), and three WAVs, each voice on its own plus a stereo merge with the tester on one channel and the bot on the other, so an overlap stays audible as two voices instead of being summed into one waveform. A test that does not leave evidence behind is just a demo.
